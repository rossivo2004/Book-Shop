// Chọn tất cả các phần tử có class là 'number-cart_quantity'
var showNumbers = document.querySelectorAll('.number-cart_quantity');
var prevButtons = document.querySelectorAll('.prev-cart_quantity');
var plusButtons = document.querySelectorAll('.plus-cart_quantity');

// Lặp qua từng phần tử để thêm sự kiện
prevButtons.forEach(function (prev, index) {
    prev.addEventListener('click', function () {
        // Check if the value is greater than zero before decrementing
        if (parseInt(showNumbers[index].value) > 0) {
            showNumbers[index].value = parseInt(showNumbers[index].value) - 1;
            // Ghi nhận giá trị mới vào phần tử tương ứng
            showNumbers[index].innerHTML = showNumbers[index].value;
        }
    });
});
...
iconSearch.addEventListener('click', function (event) {
    ...
    // Ngăn chặn sự lan truyền của sự kiện click để không ẩn phần tử ngay lập tức
    event.stopPropagation();
});

// Thêm sự kiện click cho việc ẩn
inputSearch.addEventListener('click', function (event) {
    // Ngăn chặn sự lan truyền của sự kiện click từ phần tử inputSearch lên nơi khác trên trang
    event.stopPropagation();
});

// Thêm sự kiện click lên màn hình (document) để ẩn khi nhấp vào bất kì đâu trên trang
...
ScrollReveal().reveal('.banner-stt', { interval: 400 });





var isItemsVisible = false;

        function toggleItemsVisibility() {
            var hiddenItems = document.querySelectorAll('#colorList li.hidden');

            // Show or hide the next 4 hidden items based on the current visibility state
            for (var i = 0; i < 4; i++) {
                if (hiddenItems[i]) {
                    hiddenItems[i].style.display = isItemsVisible ? 'none' : 'list-item';
                }
            }
            ...
            var buttonText = isItemsVisible ? 'Thu ngọn' : 'Xem thêm';
            var iconClass = isItemsVisible ? 'fa-chevron-up' : 'fa-chevron-down';
            document.getElementById('showMoreButton').innerHTML = buttonText + ' <i class="fa-solid ' + iconClass + '"></i>';
        }


        function toggleBoxColor(element) {
            var box = element.closest('.box-voucher_carts'); // Tìm hộp gần nhất
            var allBoxes = document.querySelectorAll('.box-voucher_carts'); // Tất cả các hộp
            allBoxes.forEach(function (item) {
                item.classList.remove('box-selected'); // Xóa mọi lớp 'box-selected'
            });
            if (element.checked) {
                box.classList.add('box-selected'); // Thêm lớp 'box-selected' khi được chọn
            }
        }